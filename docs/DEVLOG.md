...
## [2026-07-02T15:00:00Z] — arc-organ-standardize — THE ARC (medium-term context organ) + per-turn scratchpad-append discipline landed in the repo
- **what-changed:** standardized THE ARC (medium-term context organ) + per-turn scratchpad-append discipline into the self-running repo per steward directive verbatim: *"ship both — scratchpad = hygiene, ARC = centerpiece."* Every fork now inherits medium-term recall in ONE ~2-4KB cold read (`arc/ARC-NOW.md`), without re-parsing prose. Landing shape:
  1. **NEW `skills/the-arc/SKILL.md`** — the CENTERPIECE (loadable doctrine). 6-event schema (SHIFT/OPEN/CLOSE/DECIDE/VERIFY/SURPRISE); salience formula (base × exp(-ln2 · Δh / halflife) per type); composition with `scratchpad-append` (short) + `learn-cycle-contract` (long); the wake-blank verification test; anti-patterns; reversibility.
  2. **NEW `skills/scratchpad-append/SKILL.md`** — the per-turn short-term hygiene layer. Read-then-append reflex against `.claude/scratchpad-daily/YYYY-MM-DD.md`; three-step shape (read → orient → append); composition rules with `the-arc` + `learn-cycle-contract` + `auto-consolidate`; the reboot-blank verification test.
  3. **NEW `tools/arc_emit.py`** (executable) — write-gate: validates type ∈ 6, thread mandatory, evidence-mandatory-for-VERIFY/CLOSE, string ≤200 chars, ≤6 events per firewall-return; stamps salience_base + halflife_hours per type; append-only to `arc/live.jsonl`. Modes: `--type/--thread ...` (direct), `--from-firewall-return`, `--from-kanban-transition`, `--from-jsonl`. Fork config docstring updated to `{STEWARD-NAME}` + `ARC_STEWARD_ID` env note.
  4. **NEW `tools/arc_render.py`** (executable) — read-surface: `--render-now` writes `arc/ARC-NOW.md` (~2-4KB); `--diff-since <ts>` writes the delta (~1KB). Salience-ordered threads, top-3 SHIFTS, top-3 SURPRISES, HELD-FOR-STEWARD block. Origin `held_for == "corey"` compare generalized to env `ARC_STEWARD_ID` (default `steward`); docstrings + inline comments genericized.
  5. **NEW `tools/arc_compress_recent.py`** (executable) — nightly compressor: LIVE (24h) → RECENT (7d). Produces `arc/recent.md` + regenerated `arc/ARC-NOW.md`; rotates old events to `arc/_archive/`. Idempotent. No origin tokens.
  6. **NEW `tools/arc_compress_epoch.py`** (executable) — weekly compressor: RECENT (7d) → EPOCH (30d). Produces `arc/epoch.md` + regenerated `arc/ARC-NOW.md`. Idempotent. No origin tokens.
  7. **NEW `arc/README.md`** — quick-reference for the ARC organ (cold-pickup read, structure, feeds, compressors, fork config env vars, anti-patterns, wake-blank verification test). Generic — no origin state ships.
  8. **NEW `arc/_archive/.gitkeep`** — placeholder so the rotated-shards directory ships in the seed.
  9. **`INDEX.md` rewired** — new "Memory-tier stack (2, new 2026-07-02)" skills sub-table naming `scratchpad-append` (short) + `the-arc` (medium, CENTERPIECE); 4 new ARC tools added to the tools table; new `arc/` manifest section; counts updated (55 → 63 files; 12 → 14 skills; 9 → 13 tool files); version bumped to `rebuild-20260702`; delta-list bumped to 6 items.
  10. **`STAND-IT-UP.md` rewired** — anti-fossil-delta list bumped to 6 (adds item 6: the ARC + scratchpad-append pair); §2 organ-laydown adds §2d (`mkdir -p "$AICIV_ROOT/arc/_archive"` + `mkdir -p "$AICIV_ROOT/.claude/scratchpad-daily"` + `export ARC_STEWARD_ID` template line); the 30-minute path opener updated to "4 `mkdir` + 1 `touch`".
  11. **`docs/EVOLUTION-SINCE-SHIP.md` rewired** — title date bumped to 2026-07-02; delta-list header changed from "five" to "six"; §6 added with full explanation of the ARC-organ + scratchpad-append landing, PROVISIONAL status stamp, reversibility, and canonical source pointers to `data/reports/medium-term-context-organ-draft/SKILL.md` + `autonomy/skills/workflows-master/SKILL.md` §23.14-§23.15 in the origin substrate; "what a fork does with this delta" list expanded from 4 items to 5 to name the memory-tier stack read.
- **why:** the memory-tier stack was previously two named skills — short (per-turn scratchpad, informal) + long (canon via `learn-cycle-contract`). The medium tier (days-to-weeks context) was implicit — carried in scratchpad prose that decayed fast, or hoped-for in canon queries that overwhelmed the wake budget. THE ARC v2 MVP built and soaked in the origin substrate 2026-07-02 closes that gap with a structured event-stream, salience-ordered read-surface, and clean composition contracts with the adjacent tiers. Standardizing into this repo per steward directive means every fork inherits the whole picture on day 1, not just the short + long tiers.
- **files-touched (paths under exports/aiciv-self-running-repo/):**
  - skills/the-arc/SKILL.md (NEW)
  - skills/scratchpad-append/SKILL.md (NEW)
  - tools/arc_emit.py (NEW, executable — copied from origin `tools/arc_emit.py`, docstring genericized)
  - tools/arc_render.py (NEW, executable — copied from origin `tools/arc_render.py`, `held_for == "corey"` → env `ARC_STEWARD_ID`; docstrings genericized)
  - tools/arc_compress_recent.py (NEW, executable — copied from origin, no genericization needed)
  - tools/arc_compress_epoch.py (NEW, executable — copied from origin, no genericization needed)
  - arc/README.md (NEW)
  - arc/_archive/.gitkeep (NEW)
  - INDEX.md (edited — memory-tier stack sub-table; ARC tools rows; `arc/` manifest section; counts + version)
  - STAND-IT-UP.md (edited — 30-min opener; anti-fossil §6; §2d organ-laydown)
  - docs/EVOLUTION-SINCE-SHIP.md (edited — title date; §6 addition; "what a fork does" list expansion; provenance section 2026-07-02 sources added)
  - docs/DEVLOG.md (this entry)
- **.bak paths:** NONE inside the repo (branch-based reversibility — this standardization lives on a topic branch; `mneme-recommendations-wire-20260701` still at prior HEAD unchanged).
- **rollback command:** `cd $AICIV_ROOT/exports/aiciv-self-running-repo && git checkout mneme-recommendations-wire-20260701 && git branch -D arc-organ-standardize-20260702` (deletes the branch; prior HEAD untouched). Alternative fine-grained rollback: `rm -rf skills/the-arc skills/scratchpad-append arc/ tools/arc_*.py && git checkout HEAD -- INDEX.md STAND-IT-UP.md docs/EVOLUTION-SINCE-SHIP.md docs/DEVLOG.md`.
- **5-behavioral-test result:** N/A — this is a doc + tool-copy step, not a gated build step. Origin substrate carries the ARC v2 MVP live (`arc/live.jsonl` has 15KB+ of events after ~24h soak); the wake-blank verification test (fresh mind reads ONLY `arc/ARC-NOW.md`, tries to name top-5 live threads + 3 shifts + steward-held items) is scheduled after 1 week live and logs misses to `arc/miss-ledger.jsonl` on the fork. Smoke-test on this repo: `python3 tools/arc_emit.py --help` returns correct usage; `python3 tools/arc_render.py --help` returns correct usage; both compressors' `--help` return correct usage; new SKILL.md files load cleanly; INDEX + STAND-IT-UP internal links resolve.
- **proof-gate verdict:** N/A (docs + tool-copy, not gated build). Honesty gates: (a) THE ARC v2 stamped PROVISIONAL v0.1.0 — soaked in origin substrate ≥1 week before graduating to canon status; (b) the wake-blank verification test is scheduled + will log misses to `arc/miss-ledger.jsonl`; (c) origin-specific `held_for == "corey"` compare replaced with env `ARC_STEWARD_ID` (default `steward`) so no origin-steward token bleeds into a fork's ARC render; (d) reversibility named — delete `arc/` + revert workflow firewall-return schema field, no other organ depends on the ARC; (e) `scratchpad-append` stamped PROVISIONAL v0.1.0 — the discipline is cheap and honest but the whole-repo verification is the reboot-blank test on any adopter's fork after 3 days.
- **canon_append:** to be recorded in origin substrate on merge; a mind-lead canon entry will point at this DEVLOG entry + the PR URL + the origin-substrate `arc/live.jsonl` first-week soak evidence.
