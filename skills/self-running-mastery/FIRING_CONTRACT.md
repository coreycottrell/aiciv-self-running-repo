...
4. **The runbook invocable** — for a real goal, the mind can produce the exact next command (a `acg_ops_kanban_verb.py verb claim …` or a `Workflow(workflows/acg-coo.js …)` or a `canon_recall`/`canon_append` line), not a vague "I'd track it."
